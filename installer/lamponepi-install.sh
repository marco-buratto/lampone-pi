...
function System_run()
{
    if [ -n "$ISO_FILE" ] && [ -n "$DEVICE" ]; then
        printf "\n* Installing the system, please have a cup of coffee...\n"
        System_install "ISO_FILE" "$DEVICE"
...
function System_install()
{
    ...
    isoFile="/tmp/lamponepi.iso"
    if [ -f /tmp/lamponepi.iso ]; then
        rm /tmp/lamponepi.iso
    fi
    ...
    mount /home/vagrant/live-image-arm64.hybrid.iso /mnt
    ...
    xorriso -abort_on FAILURE -return_with SORRY 0 -indev "$isoFile" -boot_image any discard -overwrite on -volid 'SK-SYSTEM1' -rm_r boot efi isolinux -- -outdev stdio:${device}2 -blank as_needed

    # Create the second system partition (256MiB) and write the kernel+initrd files into.
    printf "n\np\n\n\n+256M\nw\n" | fdisk $device && sync && sleep 2
    xorriso -abort_on FAILURE -return_with SORRY 0 -indev "$isoFile" -boot_image any discard -overwrite on -volid 'SK-SYSTEM2' -rm_r boot efi isolinux -- -outdev stdio:${device}3 -blank as_needed
